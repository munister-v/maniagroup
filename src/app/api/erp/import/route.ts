...
import { parseImport, previewImport, applyImport } from "@/lib/stockImport";
...
export async function POST(req: NextRequest) {
  ...
  let parsed;
  try { parsed = parseImport(buf, file.name); }
  catch (e) { return NextResponse.json({ error: e instanceof Error ? e.message : "Не вдалося прочитати файл" }, { status: 400 }); }

  if (parsed.kind === "unknown")
    return NextResponse.json({ error: "Невідомий формат. Очікується прайс/залишки (offers) або база MG (master)." }, { status: 400 });
