...
import { saveGridChanges, type GridSaveChange } from "@/lib/erpGrid";
...
export async function POST(req: NextRequest) {
  ...
  const b = await req.json() as { changes: GridSaveChange[]; label?: string };
  if (!Array.isArray(b.changes) || !b.changes.length)
    return NextResponse.json({ error: "Немає змін" }, { status: 400 });
  try {
    const result = await saveGridChanges(b.changes, b.label);
