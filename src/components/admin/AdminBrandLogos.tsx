...
const SOURCE_LABEL: Record<string, { t: string; bg: string; c: string }> = {
  manual: { t: "Завантажено", bg: "#e8f5e9", c: "#2e7d32" },
  auto: { t: "Авто (мережа)", bg: "#e3f2fd", c: "#1565c0" },
...
export function AdminBrandLogos({ onToast }: { onToast?: (m: string) => void }) {
  ...
  async function autofill() {
    ...
    try {
      ...
      const d = await r.json();
      onToast?.(`Наповнено логотипів: ${d.filled ?? 0}`);
      await load();
      ...
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted">
            {withLogo} з {rows.length} брендів мають лого. Авто — з мережі, можна перевизначити вручну.
          </p>
        </div>
        <button
          onClick={autofill}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((r) => {
            ...
            return (
              <div key={r.brand} className="flex items-center gap-3 rounded border border-line bg-white p-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <span
                    ...
                    style={{ background: src.bg, color: src.c }}
                  >
                    {src.t}
                  </span>
                </div>
