...
async function processLogo(buf: Buffer): Promise<{ png: Buffer; bg: LogoBg } | null> {
  try {
    ...
    // Порядок важливий: classifyBg дивиться на кутові пікселі, тож обрізати
    // поля можна тільки ПІСЛЯ класифікації — інакше в кутах опиниться сам
    // малюнок і білий логотип на прозорому фоні прочитається як "dark".
    png = await trimPadding(png);
    return { png, bg };
  } catch {
    return null;
  }
...
/**
 * Зрізає однорідні поля навколо логотипа.
 *
 * Постачальники віддають логотип у квадратному полотні: напис «DOLCE&GABBANA»
 * шириною 915px живе всередині картинки 1024×1024, решта — порожнеча, тобто
 * 92% файлу. На плитці, де висота обмежена ~46px, квадрат ужимається цілком, і
 * від самого напису лишається смужка висотою 4px — саме тому логотипи здавались
 * мікроскопічними. Після обрізання ту саму висоту отримує вже напис, а не поле.
 *
 * Якщо обрізати не вдалось або результат вироджений (однотонна картинка, де
 * trim з'їдає майже все) — повертаємо оригінал: краще завеликі поля, ніж
 * порожня плитка.
 */
async function trimPadding(png: Buffer): Promise<Buffer> {
  try {
    const { data, info } = await sharp(png)
      // threshold трохи вище нуля: у PNG з антиаліасингом поля рідко бувають
      // ідеально однорідними, і за threshold=0 обрізання просто не спрацює.
      .trim({ threshold: 12 })
      .png()
      .toBuffer({ resolveWithObject: true });
    if (info.width < 16 || info.height < 16) return png;
    return data;
  } catch {
    return png;
  }
